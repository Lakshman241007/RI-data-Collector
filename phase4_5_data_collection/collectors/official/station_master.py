"""collectors/official/station_master.py – collects official IR station master data."""
...
_log = get_logger("official.station_master", "official.log")
DATASET_NAME = "station_master"

def collect(raw_dir: Path, *, source_url: str, timeout: int = 60, overwrite: bool = False) -> tuple[list[dict[str, Any]], ValidationResult]:
    """Download station master and return (records, ValidationResult)."""
    ...
    try:
        ...
        _log.error("Failed to fetch station_master: %s", exc)

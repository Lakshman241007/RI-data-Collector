"""collectors/official/train_master.py – collects official IR train master data."""
...
_log = get_logger("official.train_master", "official.log")
DATASET_NAME = "train_master"

def collect(raw_dir: Path, *, source_url: str, timeout: int = 60, overwrite: bool = False) -> tuple[list[dict[str, Any]], ValidationResult]:
    """Download train master and return (records, ValidationResult)."""
    ...
    try:
        ...
        _log.error("Failed to fetch train_master: %s", exc)

"""collectors/official/railway_divisions.py – collects official IR railway divisions data."""
...
_log = get_logger("official.railway_divisions", "official.log")
DATASET_NAME = "railway_divisions"

def collect(raw_dir: Path, *, source_url: str, timeout: int = 60, overwrite: bool = False) -> tuple[list[dict[str, Any]], ValidationResult]:
    """Download railway divisions and return (records, ValidationResult)."""
    ...
    try:
        ...
        _log.error("Failed to fetch railway_divisions: %s", exc)
